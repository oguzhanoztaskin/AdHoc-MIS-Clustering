...
The core of our work lies at the intersection of graph theory and wireless networking. A MANET can be modeled as an undirected graph $G = (V, E)$, where $V$ is the set of mobile nodes and $E$ is the set of communication links between them. A key structure for creating a network backbone is a Dominating Set (DS), which is a subset of vertices $D \subseteq V$ such that every vertex not in $D$ is adjacent to at least one vertex in $D$. This ensures full network coverage. A Maximal Independent Set (MIS) is a specific and highly useful type of dominating set. A set $I \subseteq V$ is an independent set if no two vertices in I are adjacent. It is a maximal independent set if it is an independent set that is not a proper subset of any other independent set. Every MIS is, by definition, also a dominating set. This property makes it an ideal candidate for clustering, as the selected CHs (the nodes in the MIS) are guaranteed to cover the network while also being spatially separated, which is beneficial for reducing interference and reusing wireless channel resources.
